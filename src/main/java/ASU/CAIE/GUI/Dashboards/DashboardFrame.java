...
import static ASU.CAIE.Users.Role.PROFESSOR;
import static ASU.CAIE.Users.Role.STUDENT;
...
public class DashboardFrame {
    ...
    private void switchContent(String section) {
        ...
        Supplier<Node> newSupplier = () -> switch (section) {
            ...
            case "Overview"     -> switch (currentUser.getRole()) {
                ...
                case INSTRUCTOR, PROFESSOR -> new InstructorDashboardView().build();
                case ADMIN, STAFF          -> new AdminDashboardView().build();
                case PARENT                -> new ParentDashboardView().build();
    ...
    private HBox buildTopBar() {
        ...
        mockBtn.setPrefHeight(32);
        mockBtn.setStyle(
                "-fx-background-color: " + (AppConfig.USE_MOCK_DATA ? "#dcfce7" : "#f3f4f6") + ";" +
                        "-fx-text-fill: "        + (AppConfig.USE_MOCK_DATA ? "#16a34a" : "#6b7280") + ";" +
                        "-fx-font-size: 11px;" +
                        "-fx-font-weight: bold;" +
                        "-fx-background-radius: 6;" +
                        "-fx-border-radius: 6;" +
                        "-fx-border-color: "     + (AppConfig.USE_MOCK_DATA ? "#86efac" : "#e5e7eb") + ";" +
                        "-fx-border-width: 1;" +
                        "-fx-cursor: hand;" +
        ...
        topBar.getChildren().addAll(pageTitle, spacer, mockBtn, themeToggle);
        return topBar;
    }
}